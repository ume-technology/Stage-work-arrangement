...
@Read:基于该数据统计的基础思路：优化师/产品/优化师投放的产品/广告/
        2.然后读取了所有投放在FB平台的产品，匹配了这些优化师投放在FB平台的都有哪些产品（无月份依赖）；
        3.最后读取了这些Top优化师投放出来的广告系列都有哪些（无月份依赖）；
"""
from nowaddata.read_gdsc_gk_product_report.read_gdsc_gk_product_report import gk_product_reporter_df, \
    ader_id_groups  # 获取优化师投放的广告涉及的商品的report
import pickle
import pandas as pd
...
with open('../../bigfiles/unified_joined_new/all.pick', 'rb') as f:
    tags = pickle.load(f)
with open('../../bigfiles/unified_joined_new/all_no.pick', 'rb') as f:
    tags_no = pickle.load(f)

# todo 确定优化师投放的商品   202211月份 - 筛选特定家族的优化师
# 首先读取了各个家族的Top优化师s的ID数据；（这里有一个依赖是每个月各团队的Top优化师s可能会不同，因此要注意月份） - 该方案读取优化师的过程已经废弃
...
# todo 该数据文件由navicat执行导出 - 11月度下的订单数据 - 进而也就同步确定了top优化师的数据
# 1、获取各家族FB平台各月订单量TOP5优化师名下的广告系列id
# 2、所需字段：产品id、商品id、广告系列id、广告系列名称、出单量、订单日期、出单地区
data = pd.read_csv('../../bigfiles/read_tb_dwd_ord_gk_oder_info_crt_df_standard_facebook_202211.csv')
...
# all_opts_orders_thismonth_prtorders_count = \
#     all_opts_orders_thismonth.groupby(by=['line_name', 'product_id'])['sum(is_effective_order)'].sum().to_frame()
# all_opts_orders_thismonth_prtorders_count = \
#     all_opts_orders_thismonth.groupby(by=['line_name', 'product_id'])['sum(is_effective_order)'].sum()
# 分组求和
all_opts_orders_thismonth = all_opts_orders_thismonth.groupby(['line_name', 'product_id']). \
    agg({"sum(is_effective_order)": "sum"})
# 持续分割字段
